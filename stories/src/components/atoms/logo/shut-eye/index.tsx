...
export const ShutEye: React.FC = () => {
  ...
  const useDarkColour = window.matchMedia(
    "(prefers-color-scheme: dark)",
  ).matches;
  const genericColour = useDarkColour ? "white" : "black";
  const colour = canvasRef.current
    ? getComputedStyle(canvasRef.current as HTMLCanvasElement).color
    : "currentColor";
  ...
  useEffect(() => {
    requestAnimationFrame(() => {
      // 0 = Open eye = active
      // 1 = Closed eye
      generateCanvasEye(Number(progress), canvasRef.current, colour);

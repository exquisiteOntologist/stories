...
export const fetchPhrasesToCollection = createAsyncThunk("collectionToPhrase/fetchCollectionToPhrase", async (collectionId: number, { dispatch }) => {
    try {
        const phrases: PhraseResult[] = (await invoke("collection_phrases_today", {
            collectionId,
        })) as PhraseResult[];
        ...
        const collectionToPhrases: CollectionToPhrase[] = phrases.map((p) => ({ collection_id: collectionId, phrase_id: p.id }) as CollectionToPhrase);
        ...
        console.error("Unable to fetch Collection to Phrase mappings for", collectionId, e);
...
const collectionToPhraseAdapter = createEntityAdapter<CollectionToPhrase>({
    selectId: (collectionToPhrase) => (collectionToPhrase.collection_id, collectionToPhrase.phrase_id),
});
...
export const { setAllCollectionToPhrase, addManyCollectionToPhrase, upsertCollectionToPhrase, removeCollectionToPhrase } = collectionToPhraseSlice.actions;
export const collectionToPhraseSelectors = collectionToPhraseAdapter.getSelectors<RootState>((state) => state.collectionToPhrase);
...
export const selectNestedPhraseIds = createSelector(
    ...
    (cToSs, s) => {
        ...
        const nestedPhrases = cToSs.filter((cToS) => currentCollectionId === cToS.collection_id);
        const nestedPhraseIds: number[] = nestedPhrases.map((cToC) => cToC.phrase_id);

        ...
        <div className={`${!!searchResults ? "opacity-100" : "opacity-0"} transition-opacity duration-150`}>
            ...
            {sr.sources.length ? (
                <div className="grid grid-cols-3 gap-4 mb-12">
                    {sr.sources.map((s) => (
                        <ListingRow key={s.id} id={s.id} title={s.name} action={() => console.info("selecting sources not supported yet")} />
                    ))}
                </div>
            ) : null}
            ...
            {sr.collections.length ? (
                <div className="grid grid-cols-3 gap-4 mb-12">
                    {sr.collections.map((c) => (
                        <ListingRow key={c.id} id={c.id} title={c.name} action={() => dispatch(chooseCollection(c.id))} />
                    ))}
                </div>
            ) : null}
            ...
            {sr.contents.length ? (
                <div className="grid grid-cols-3 gap-4 mb-12">
                    {sr.contents.map((c) => (
                        <ListingRow key={c.id} id={c.id} title={c.title} linkUrl={c.url} />
                    ))}
                </div>
            ) : null}

  ...
  return (
    ...
    >
      {source.name.split(" ").map((word, i) => (
        <span
          key={i}
          className="inline-block first-letter:text-gray-500 dark:first-letter:text-gray-300 group-[.source-link]:group-hover:first-letter:text-gray-600 dark:group-[.source-link]:group-hover:first-letter:text-gray-200"
        >
          {`${word}`}&nbsp;
        </span>
      ))}

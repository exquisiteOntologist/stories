...
export const RefreshBar: React.FC<RefreshBarProps> = ({ refreshAction: action, refreshPossible }) => (
    <AnimatePresence>
        {refreshPossible && (
            <motion.nav {...motionPropsRefreshBar} onClick={action} className="cursor-pointer w-2/3 max-w-xl px-7 py-2 z-50 fixed bottom-10 left-1/2 -translate-x-1/2 rounded-full border-b border-transparent bg-black text-white shadow-md shadow-white dark:shadow-black">

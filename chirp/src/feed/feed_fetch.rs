...
pub async fn feed_fetch(source_id: i32, url: String, other_param: &String) -> Result<(Source, Vec<Contents>), Box<dyn Error + Send + Sync>> {
	...
	let parse_result: Result<(Source, Vec<Contents>), Box<dyn Error + Send + Sync>> = if is_atom {

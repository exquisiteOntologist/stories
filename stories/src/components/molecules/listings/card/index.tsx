...
export const ListingCard: React.FC<ListingCardProps> = ({
  ...
  const listingCoverImage = content?.media?.length
    ? [...content?.media][0]
    : undefined; //?.sort(m => m.isCover ? 0 : 1)[0]?.mediaImage[0]
  const perfectFit = (pxSize?: number) =>
    pxSize ? `min(100%, ${pxSize}px)` : "100%";
  ...
  return (
    <motion.article
      {...motionProps}
      className="group relative flex flex-col text-center mb-6"
    >
      ...
      <h1 className="grow mx-0 my-2 text-sm">
        <a
          className="grow font-bold text-current dark:text-slate-300 opacity-70 group-hover:opacity-100 transition-all duration-50"
          href={linkUrl}
          target="_blank"
        >
          {title}

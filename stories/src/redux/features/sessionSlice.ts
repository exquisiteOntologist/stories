...
export const checkRetrievalsIsUpdating = createAsyncThunk(
  "session/retrievalsIsUpdating",
  async (_, { dispatch }) => {
    dispatch(
      setHasRecentUpdates(await invoke<boolean>("retrievals_is_updating")),
    );
  },
);

export const ticker = createAsyncThunk(
  "session/ticker",
...
export const selectSession = (s: RootState): Readonly<RootState["session"]> =>
  s.session;
export const selectLaunchDate = (s: RootState): string => s.session.launched;
export const selectTimeSinceLaunch = (s: RootState): number =>
  minutesSince(s.session.launched);
/** Time frame since launch that another update was expected to be done by */
export const selectShouldHaveUpdated = (s: RootState): boolean =>
  selectTimeSinceLaunch(s) > 15;
/** Has the server recorded a retrieval attempt recently? */
export const selectHasUpdated = (s: RootState): boolean =>
  s.session.hasRecentUpdates;
...
export const selectIsInitializing = (s: RootState): boolean =>
  !selectHasUpdated(s) && selectTimeSinceLaunch(s) < 1;
/** Is loading? */
export const selectIsLoading = (s: RootState): boolean =>
  selectIsInitializing(s);

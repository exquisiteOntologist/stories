...
export const ListingsContainerPhrase: React.FC<ListingsContainerPhraseProps> = ({ className, view, phrases, selectAction }) => (
    <ListingsContainer className={className} view={SettingsLayout.CARDS /* view */}>
        {phrases.map((p) => (
            <ListingRow key={p.id} id={p.id} title={p.phrase} action={() => console.log("phrase", p)} bold={true} source={{ id: 0, name: "Phrase", url: "", site_url: "", kind: SourceKind.WEB }} />
        ))}

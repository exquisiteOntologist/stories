...
import { selectContentByRecency, selectContentOfCollection } from "../../../redux/features/contentsSlice";
...
export const CollectionCustomizer: React.FC<CollectionCustomizerProps> = ({ collectionSettings, isCustomizing }) => {
    ...
    const otherLayoutOption = viewIsList ? SettingsLayout.CARDS : SettingsLayout.ROWS;

    return (
        <div className={`z-50 transition-all duration-100 ${isCustomizing ? "opacity-1 translate-x-0 translate-y-0" : "opacity-0 translate-x-3 -translate-y-3"} mb-6`}>
            <div className={`flex justify-start`}>
                <Button label="Done" Icon={IconTickCircle} action={() => dispatch(setIsCustomizing(false))} />
                <Button
                ...
                >
                    <PopoverViewMode contents={previewContents} sources={sources} layout={otherLayoutOption} />
                </Button>
                <Button label="Add Widget" Icon={IconAddCircle} action={() => void 8} disabled={true} />

...
export const AddCollection: React.FC = () => {
  ...
  const [newCollectionMessage, setNewCollectionMessage] = useState<
    [string, boolean]
  >(["", false]);
  ...
  const submitAddCollection = async (e: React.FormEvent) => {
    ...
    setNewCollectionMessage([
      `${success ? "Succeeded" : "Failed"} adding new collection "${newCollectionName}"`,
      !success,
    ]);
    if (success) setNewCollectionName("");
  };

  const [newCollectionMessageText, newCollectionMessageError] =
  ...
  return (
    <div className="mb-10">
      ...
      {newCollectionMessageText ? (
        <p
          className={`${newCollectionMessageError ? "text-orange-700" : "text-green-700"}`}
        >
          {newCollectionMessageText}&nbsp;
        </p>
      ) : (
        <Hint
          title="Note"
          text="The new collection will be nested within the current collection."
        />
      )}
    </div>
  );

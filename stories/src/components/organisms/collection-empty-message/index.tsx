...
export const CollectionEmptyMessage: React.FC = () => {
  ...
  const hasNestedCollections = !!useAppSelector(selectNestedCollections)
    ?.length;
  const hasContents = !!useAppSelector(selectContentOfCollection)?.length;
  const isLoading = !!useAppSelector(selectLoading);

  const notEmpty =
    !isLoading && !!collection && !hasNestedCollections && !hasContents;
  ...
  return (
    <motion.div {...motionProps}>
      <H2>
        Add Something to <Light colour="yellow">{collection?.name}</Light>
      </H2>
      <p className="text-current mb-6">
        This collection is empty. There are no sources &amp; no nested
        collections.
      </p>
      <div className="flex">
        <FloatingButton

...
pub async fn update() -> Result<(), Box<dyn Error>> {    
    let sources = db_sources_retrieve_outdated()?;
...
pub async fn update_single_feed(source: &Source) -> Result<(), Box<dyn Error>> {
    ...
    db_source_retrievals_update_success(&source.id)?;

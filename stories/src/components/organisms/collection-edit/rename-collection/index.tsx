...
export const RenameCollectionSection: React.FC = () => {
  ...
  const [renameCollectionMessage, setRenameCollectionMessage] = useState<
    [string, boolean]
  >(["", false]);
  ...
  const submitRenameCollection = async (e: React.FormEvent) => {
    ...
    setRenameCollectionMessage([
      `${success ? "Finished renaming" : "Failed to rename"} collection "${renameCollectionName}"`,
      !success,
    ]);
    dispatch(fetchSourcesOfCollection([collectionId]));
  };

  const [renameCollectionMessageText, renameCollectionMessageError] =
    ...
    <div className="mb-10">
      ...
      <form className="flex mb-2" onSubmit={submitRenameCollection}>
        <Field
          ...
          updater={setRenameCollectionName}
        />
        <FloatingButton
          className={`${buttonClassesHollow} whitespace-nowrap`}
          action={() => {}}
          label="Rename"
          disabled={!renameCollectionName}
        />
      </form>
      {renameCollectionMessageText ? (
        <p
          className={`${renameCollectionMessageError ? "text-orange-700" : "text-green-700"}`}
        >
          {renameCollectionMessageText}&nbsp;
        </p>
      ) : (

        ...
        <>
            ...
            >
                ...
                {showOriginalArticle && content.url ? (
                    <iframe className={`w-[90vw] h-[80vh] mx-auto ${originalArticleLoaded ? "block" : "hidden"} border-2 border-solid border-slate-400`} style={{ borderColor: colours.primaryHslVals && createHslaString(...colours.primaryHslVals, 0.5) }} src={content.url} onLoad={(e) => setOriginalArticleLoaded(!!(e.target as HTMLIFrameElement)?.contentWindow?.window.length)}></iframe>
                ) : (
                    <section className={`reader__body w-full max-w-5xl px-4 mb-2 mx-auto ${coverImageUrl ? "has-cover" : ""}`} dangerouslySetInnerHTML={{ __html: articleHtml }}></section>
                )}
            </article>
        </>
    );

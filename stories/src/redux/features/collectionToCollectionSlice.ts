...
export const fetchCollectionToCollection = createAsyncThunk(
  ...
  async (parentCollectionIds: number[], { dispatch }) => {
    try {
      const collectionToCollectionItems = await invoke(
        "get_collection_to_collection",
        {
      ...
      dispatch(
        upsertCollectionToCollection(
          collectionToCollectionItems as CollectionToCollection[],
        ),
      );
    } catch (e) {
      console.error(
        "Unable to fetch Collection to Collection mappings for",
        parentCollectionIds,
        e,
      );
...
const collectionToCollectionAdapter = createEntityAdapter({
  selectId: (collectionToCollection: CollectionToCollection) => (
    collectionToCollection.collection_parent_id,
    collectionToCollection.collection_inside_id
  ),
});
...
export const {
  setAllCollectionToCollection,
  addManyCollectionToCollection,
  upsertCollectionToCollection,
  removeCollectionToCollection,
} = collectionToCollectionSlice.actions;
export const collectionToCollectionSelectors =
  collectionToCollectionAdapter.getSelectors<RootState>(
    (state) => state.collectionToCollection,
  );
...
export const selectNestedCollectionIds = createSelector(
  ...
  (cToCs, s) => {
    ...
    const nestedCollections = cToCs.filter(
      (cToC) => currentCollectionId === cToC.collection_parent_id,
    );
    const nestedCollectionIds: number[] = nestedCollections.map(
      (cToC) => cToC.collection_inside_id,
    );
...
export const collectionToCollectionReducer =
  collectionToCollectionSlice.reducer;

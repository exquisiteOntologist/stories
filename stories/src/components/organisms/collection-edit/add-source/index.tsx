...
export const AddSource: React.FC = () => {
  ...
  const [addSourceMessage, setAddSourceMessage] = useState<[string, boolean]>([
    "",
    false,
  ]);
  ...
  const submitAddToCollection = async (e: React.FormEvent) => {
    ...
    const success = (
      await dispatch(
        addSourceToCollection({
          collectionId,
          sourceUrl: sourceUrlEntry,
          otherParam: otherParamEntry,
        }),
      )
    ).payload;
    setAddSourceMessage([
      `${success ? "Finished adding" : "Failed to add"} source "${sourceUrlEntry}"`,
      !success,
    ]);
    ...
    <div className="mb-10">
      ...
      <form className="flex mb-2" onSubmit={submitAddToCollection}>
        <Field
          ...
          value={sourceUrlEntry}
          updater={setSourceUrlEntry}
        />
        <Field
          placeholder="Article URL '/segment/'"
          value={otherParamEntry}
          updater={setOtherParamEntry}
        />
        <FloatingButton
          className={`${buttonClassesHollow} whitespace-nowrap`}
          action={() => {}}
          label="Add"
          disabled={!sourceUrlEntry}
        />
      </form>
      {addMessageText ? (
        <p
          className={`${addMessageWasError ? "text-orange-700" : "text-green-700"}`}
        >
          {addMessageText}&nbsp;
        </p>
      ) : (
        <Hint
          title="Tip"
          text="If articles on a site have &ldquo;/2023/&rdquo; in their URLs, add &ldquo;2023&rdquo; as the segment.&nbsp;"
        />
      )}
    </div>
  );

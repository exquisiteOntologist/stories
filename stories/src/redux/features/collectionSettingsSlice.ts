...
export const fetchCollectionSettings = createAsyncThunk(
  ...
  async (collection_ids: number[], { dispatch }) => {
    ...
    dispatch(
      upsertCollectionSettings(collectionSettings as CollectionSettings[]),
    );
  },
);
...
const collectionSettingsAdapter = createEntityAdapter({
  selectId: (collectionSettings: CollectionSettings) =>
    collectionSettings.collection_id,
});
...
export const {
  setAllCollectionSettings,
  addCollectionSettings,
  upsertCollectionSettings,
} = collectionSettingsSlice.actions;
export const collectionSettingsSelectors =
  collectionSettingsAdapter.getSelectors<RootState>(
    (state) => state.collectionSettings,
  );

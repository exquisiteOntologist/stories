...
import {
  chooseCollection,
  selectHistory as selectHistoryIds,
} from "../../../redux/features/navSlice";
import { TitleCrumbsProps } from "./interfaces";

export const TitleCrumbs: React.FC<TitleCrumbsProps> = ({
  collectionId,
  title,
}) => {
  const dispatch = useAppDispatch();
  const submergeHistoryIds = useAppSelector(selectHistoryIds);
  const submergeHistoryItems = useAppSelector((s) =>
    submergeHistoryIds.map((id) => collectionsSelectors.selectById(s, id)),
  ).filter((x) => typeof x !== "undefined") as Collection[];
  ...
  const historyItems = submergeHistoryItems.map((hi) => {
    ...
    return (
      <span
        key={hi.id}
        className={`text-current ${cursorStyle} mr-3 ${colour}`}
        onClick={() => dispatch(chooseCollection(hi.id))}
      >
        {hi.name}
      </span>
    );

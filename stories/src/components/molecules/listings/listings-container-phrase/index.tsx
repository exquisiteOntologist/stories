...
export const ListingsContainerPhrase: React.FC<
  ListingsContainerPhraseProps
> = ({ className, view, phrases }) => (
  <ListingsContainer
    className={className}
    view={SettingsLayout.COLUMNS /* view */}
  >
    {phrases.map((p) => (
      <ListingRow
        key={p.id}
        id={p.id}
        title={`${p.phrase} (${p.total.toLocaleString()})`}
        action={() => console.log("phrase", p)}
        bold={true}
        source={{
          id: 0,
          name: "Phrase",
